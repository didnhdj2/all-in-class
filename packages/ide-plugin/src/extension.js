...
var require_iterator = __commonJS({
  "../../node_modules/vscode-languageclient/node_modules/yallist/iterator.js"(exports, module2) {
    "use strict";
...
var require_yallist = __commonJS({
  "../../node_modules/vscode-languageclient/node_modules/yallist/yallist.js"(exports, module2) {
    "use strict";
...
var require_lru_cache = __commonJS({
  "../../node_modules/vscode-languageclient/node_modules/lru-cache/index.js"(exports, module2) {
    "use strict";
...
// ../../node_modules/vscode-languageserver-protocol/node_modules/vscode-languageserver-types/lib/umd/main.js
var require_main2 = __commonJS({
  "../../node_modules/vscode-languageserver-protocol/node_modules/vscode-languageserver-types/lib/umd/main.js"(exports, module2) {
    (function(factory) {
...
var require_client = __commonJS({
  ...
  Code: ${responseError.code} ${responseError.data ? "\n" + responseError.data.toString() : ""}`;
      ...
      error(message, data, showNotification = true) {
        ...
        if (showNotification && this._clientOptions.revealOutputChannelOn <= RevealOutputChannelOn2.Error) {
          this.showNotificationMessage();
        }
      }
    ...
    var FileOperationFeature = class {
      ...
      register(data) {
        ...
        const minimatchFilter = data.registerOptions.filters.map((filter) => {
          const matcher = new minimatch2.Minimatch(filter.pattern.glob, FileOperationFeature.asMinimatchOptions(filter.pattern.options));
          if (!matcher.makeRe()) {
...
// ../tailwindcss-language-service/src/util/languages.ts
var htmlLanguages = [
  "aspnetcorerazor",
  "astro",
  "astro-markdown",
...
// ../tailwindcss-language-service/src/util/isObject.ts
function isObject(variable) {
  return Object.prototype.toString.call(variable) === "[object Object]";
}

// ../tailwindcss-language-service/src/util/array.ts
function dedupe(arr) {
...
// utils/log.ts
var LogRedirectOutputChannel = class {
  constructor(name) {
    this.name = name;
  }
  append(value) {
...
// src/extension.ts
var colorNames = Object.keys(import_color_name.default);
var CLIENT_ID = "tailwindcss-intellisense";
var CLIENT_NAME = "Tailwind CSS IntelliSense";
var CONFIG_FILE_GLOB = "{vite,vite.config}.{js,cjs}";
var clients = /* @__PURE__ */ new Map();
var languages2 = /* @__PURE__ */ new Map();
var searchedFolders = /* @__PURE__ */ new Set();
var _sortedWorkspaceFolders;
function sortedWorkspaceFolders() {
  if (_sortedWorkspaceFolders === void 0) {
    _sortedWorkspaceFolders = import_vscode.workspace.workspaceFolders ? import_vscode.workspace.workspaceFolders.map((folder) => {
...
function getOuterMostWorkspaceFolder(folder) {
  ...
  for (let element of sorted) {
    ...
    if (uri.startsWith(element)) {
      return import_vscode.workspace.getWorkspaceFolder(import_vscode.Uri.parse(element));
    }
  }
  return folder;
}
function getUserLanguages(folder) {
  let test = import_vscode.workspace.getConfiguration("tailwindCSS", folder);
  const langs = import_vscode.workspace.getConfiguration("tailwindCSS", folder).includeLanguages;
  return isObject(langs) ? langs : {};
}
function getGlobalExcludePatterns(scope) {
  return Object.entries(import_vscode.workspace.getConfiguration("files", scope).get("exclude")).filter(([, value]) => value === true).map(([key]) => key).filter(Boolean);
}
function getExcludePatterns(scope) {
  return [
    ...getGlobalExcludePatterns(scope),
    ...["**/.git/**", "**/node_modules/**", "**/.hg/**"].filter(Boolean)
  ];
}
function isExcluded(file, folder) {
  let exclude = getExcludePatterns(folder);
  for (let pattern of exclude) {
    if ((0, import_minimatch.default)(file, path.join(folder.uri.fsPath, pattern))) {
      return true;
    }
  }
  return false;
...
function mergeExcludes(settings, scope) {
  return __spreadProps(__spreadValues({}, settings), {
    files: __spreadProps(__spreadValues({}, settings.files), {
...
function activate(context) {
  return __async(this, null, function* () {
    let module2 = context.asAbsolutePath(path.join("dist", "server.js"));
    console.log('==== module2 :', module2);
    let prod = path.join("dist", "tailwindServer.js");
    console.log('==== prod :', prod);
    try {
			console.log('==== import_vscode.Uri.joinPath(context.extensionUri, prod) :', import_vscode.Uri.joinPath(context.extensionUri, prod));
      yield import_vscode.workspace.fs.stat(import_vscode.Uri.joinPath(context.extensionUri, prod));
      module2 = context.asAbsolutePath(prod);
    } catch (_) {
    }
    let outputChannel;
    context.subscriptions.push(import_vscode.commands.registerCommand("tailwindCSS.showOutput", () => {
      if (outputChannel) {
        outputChannel.show();
      }
    }));
    let watcher = import_vscode.workspace.createFileSystemWatcher(`**/${CONFIG_FILE_GLOB}`, true, true, true);
    watcher.onDidCreate((uri) => {
      let folder = import_vscode.workspace.getWorkspaceFolder(uri);
    ...
    context.subscriptions.push(import_vscode.workspace.onDidChangeConfiguration((event) => {
			console.log('====  onDidChangeConfiguration:', event);
      ;
      [...clients].forEach(([key, client]) => {
        const folder = import_vscode.workspace.getWorkspaceFolder(import_vscode.Uri.parse(key));
        let reboot = false;
    ...
    function bootCssServer() {
      return __async(this, null, function* () {
        ...
        let module3 = context.asAbsolutePath(path.join("dist", "cssServer.js"));
        let prod2 = path.join("dist", "tailwindModeServer.js");
        try {
          yield import_vscode.workspace.fs.stat(import_vscode.Uri.joinPath(context.extensionUri, prod2));
          module3 = context.asAbsolutePath(prod2);
        } catch (_) {
        }
        let client = new import_node.LanguageClient("tailwindcss-intellisense-css", "Tailwind CSS", {
          run: {
            module: module3,
          ...
          middleware: {
            provideCompletionItem(document, position, context2, token, next) {
              function updateRanges(item) {
                ...
                if (range instanceof import_vscode.Range && range.end.isAfter(position) && range.start.isBeforeOrEqual(position)) {
                  item.range = { inserting: new import_vscode.Range(range.start, position), replacing: range };
                }
              }
              function updateLabel(item) {
                if (item.kind === import_vscode.CompletionItemKind.Color) {
                  item.label = {
                    label: item.label,
              ...
              function updateProposals(r2) {
                if (r2) {
                  ;
                  (Array.isArray(r2) ? r2 : r2.items).forEach(updateRanges);
                  (Array.isArray(r2) ? r2 : r2.items).forEach(updateLabel);
                }
                return r2;
              }
              const isThenable = (obj) => obj && obj["then"];
              const r = next(document, position, context2, token);
              if (isThenable(r)) {
        ...
        function initCompletionProvider() {
          ...
          return import_vscode.languages.registerCompletionItemProvider(["tailwindcss"], {
            provideCompletionItems(doc, pos) {
              let lineUntilPos = doc.getText(new import_vscode.Range(new import_vscode.Position(pos.line, 0), pos));
              let match = lineUntilPos.match(regionCompletionRegExpr);
              if (match) {
                let range = new import_vscode.Range(new import_vscode.Position(pos.line, match[1].length), pos);
                let beginProposal = new import_vscode.CompletionItem("#region", import_vscode.CompletionItemKind.Snippet);
                beginProposal.range = range;
                import_vscode.TextEdit.replace(range, "/* #region */");
                beginProposal.insertText = new import_vscode.SnippetString("/* #region $1*/");
                beginProposal.documentation = "Folding Region Start";
                beginProposal.filterText = match[2];
                beginProposal.sortText = "za";
                let endProposal = new import_vscode.CompletionItem("#endregion", import_vscode.CompletionItemKind.Snippet);
    ...
    function bootWorkspaceClient(folder) {
      ...
      if (!outputChannel) {
        outputChannel = import_vscode.window.createOutputChannel(CLIENT_NAME);
        context.subscriptions.push(outputChannel);
        import_vscode.commands.executeCommand("setContext", "tailwindCSS.hasOutputChannel", true);
      }
      let configuration = {
        editor: import_vscode.workspace.getConfiguration("editor", folder),
        tailwindCSS: mergeExcludes(import_vscode.workspace.getConfiguration("tailwindCSS", folder), folder)
      };
      ...
      let clientOptions = {
        documentSelector: languages2.get(folder.uri.toString()).concat(["html_es6"]).map((language) => ({
          scheme: "file",
          language,
        ...
        outputChannel: new LogRedirectOutputChannel("tailwind Language Server"),
        revealOutputChannelOn: import_node.RevealOutputChannelOn.Never,
        middleware: {
          resolveCompletionItem(item, token, next) {
            return __async(this, null, function* () {
              var _a;
              let result = yield next(item, token);
              let selections = import_vscode.window.activeTextEditor.selections;
              if (selections.length > 1 && ((_a = result.additionalTextEdits) == null ? void 0 : _a.length) > 0) {
                let length = selections[0].start.character - result.additionalTextEdits[0].range.start.character;
                let prefixLength = result.additionalTextEdits[0].range.end.character - result.additionalTextEdits[0].range.start.character;
                let ranges = selections.map((selection) => {
                  return new import_vscode.Range(new import_vscode.Position(selection.start.line, selection.start.character - length), new import_vscode.Position(selection.start.line, selection.start.character - length + prefixLength));
                });
                if (ranges.map((range) => import_vscode.window.activeTextEditor.document.getText(range)).every((text, _index, arr) => arr.indexOf(text) === 0)) {
          ...
          provideDocumentColors(document, token, next) {
            return __async(this, null, function* () {
              var _a;
              let colors = yield next(document, token);
              let editableColors = colors.filter((color) => {
                var _a2, _b;
                let text = (_b = (_a2 = import_vscode.workspace.textDocuments.find((doc) => doc === document)) == null ? void 0 : _a2.getText(color.range)) != null ? _b : "";
                return new RegExp(`-\\[(${colorNames.join("|")}|((?:#|rgba?\\(|hsla?\\())[^\\]]+)\\]$`).test(text);
              });
              let nonEditableColors = colors.filter((color) => !editableColors.includes(color));
              if (!colorDecorationType) {
                colorDecorationType = import_vscode.window.createTextEditorDecorationType({
                  before: {
                    width: "0.8em",
                    height: "0.8em",
                    contentText: " ",
              ...
              (_a = import_vscode.window.visibleTextEditors.find((editor) => editor.document === document)) == null ? void 0 : _a.setDecorations(colorDecorationType, nonEditableColors.map(({ range, color }) => ({
                range,
                renderOptions: {
                  before: {
          ...
          workspace: {
            configuration: (params) => {
              return params.items.map(({ section, scopeUri }) => {
                let scope = folder;
                if (scopeUri) {
                  let doc = import_vscode.workspace.textDocuments.find((doc2) => doc2.uri.toString() === scopeUri);
                  if (doc) {
                    scope = {
      ...
      let client = new import_node.LanguageClient(CLIENT_ID, CLIENT_NAME, serverOptions, clientOptions);
      client.onReady().then(() => {
        client.onNotification("@/tailwindCSS/error", (_0) => __async(this, [_0], function* ({ message }) {
          let action = yield import_vscode.window.showErrorMessage(message, "Go to output");
          if (action === "Go to output") {
            import_vscode.commands.executeCommand("tailwindCSS.showOutput");
          }
        }));
        client.onNotification("@/tailwindCSS/clearColors", () => clearColors());
        client.onRequest("@/tailwindCSS/getDocumentSymbols", (_0) => __async(this, [_0], function* ({ uri }) {
          return import_vscode.commands.executeCommand("vscode.executeDocumentSymbolProvider", import_vscode.Uri.parse(uri));
        }));
      });
      client.onDidChangeState(({ newState }) => {
        if (newState === import_node.State.Stopped) {
          clearColors();
        }
      });
      client.start();
    ...
    function didOpenTextDocument(document) {
      return __async(this, null, function* () {
				console.log('==== didOpenTextDocument .languageId :', document.languageId);
        ...
        const dirName = folder.uri.fsPath;
        const configFile = dirName + "/tailwind.config.js";
        console.log('==== didOpenTextDocument configFile :', configFile);
    ...
    context.subscriptions.push(import_vscode.workspace.onDidOpenTextDocument(didOpenTextDocument));
    import_vscode.workspace.textDocuments.forEach(didOpenTextDocument);
    context.subscriptions.push(import_vscode.workspace.onDidChangeWorkspaceFolders((event) => {
