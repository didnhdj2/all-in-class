...
async function genProvideItem(config) {
	...
	gendDynamicRules = dynamicRules
	res = Object.values(dynamicRules).map(([key, fun]) => {
		let resss = new Reg2str(key).setMax(0).gen()
		...
		return {
			...
			documentation: {
				kind: 'markdown',
				value: fun(resss.match(key), { unit: 'px' })
			},
			sortText: resss,
			filterText: resss,
			insertText: { _tabstop: 1, value: resss }
		}
	});

	res = res.concat(Object.entries(staticRules).map(([key, val]) => {
		return {
			label: key,
			kind: 11,
			detail: '对应的样式：',
...
export function provideHover(document, position, token) {
	...
	if (!contents) {
		if (provideItemCache[word]) {
			contents = provideItemCache[word]
		} else {
			// 使用动态规则来匹配
			Object.values(gendDynamicRules).find(([rule, fun]) => {
				let match = word.match(rule)
				if (!match) return
				
				let cssVal = fun(match)
				contents = {
					label: word,
					kind: 11,
					detail: '对应的样式为：',
					documentation: { kind: 'markdown', value: cssVal },
					sortText: word,
					filterText: word,
					insertText: { _tabstop: 1, value: word }
				}

				if (!provideItemCache[word]) {
					provideItem.push(contents)
					provideItemCache[word] = contents
				}
	...
	console.log('==== dynamicRules.find(word) :', contents);
	if (!contents) {
		return new Promise(resolve => {
			resolve(null);
		});
...
function registerCompletionItemProvider() {
	...
	let provideCompletionItems = (document, position) => {
		...
		console.log('==== 1111 provideCompletionItems isInClass:', !isInClass(document, position));
		if (!isInClass(document, position)) return { items: [] }

		// 获取当前的输入
		const wordRange = document.getWordRangeAtPosition(position, /[^\s'"]+/);
		const word = document.getText(wordRange)
		console.log('==== word provideItem:', word);
		if (!word || word.length > 50) return { items: [] }
