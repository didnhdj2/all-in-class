  ...
  return {
    rules: [
      ...
      [
        /^mlr-(\d+)$/,
        (match, { unit }) =>
          `margin-left:${match[1]}${unit};margin-right:${match[1]}${unit};`,
      ],
      [
        /^mtb-(\d+)$/,
        (match, { unit }) =>
          `margin-top:${match[1]}${unit};margin-bottom:${match[1]}${unit};`,
      ],
      [
        /^m-(\d+)-(\d+)-(\d+)-(\d+)$/,
        (match) => ({
      ...
      [
        /^pt-(\d+)$/,
        (match, { unit }) => ({ "padding-top": `${match[1]}${unit}` }),
      ],
      [
        /^pb-(\d+)$/,
        (match, { unit }) => ({ "padding-bottom": `${match[1]}${unit}` }),
      ],
      [
        /^pl-(\d+)$/,
        (match, { unit }) => ({ "padding-left": `${match[1]}${unit}` }),
      ],
      [
        /^pr-(\d+)$/,
        (match, { unit }) => ({ "padding-right": `${match[1]}${unit}` }),
      ],
      [
        /^ptb-(\d+)$/,
        (match, { unit }) =>
          `padding-top:${match[1]}${unit};padding-bottom:${match[1]}${unit};`,
      ],
      [
        /^plr-(\d+)$/,
        (match, { unit }) =>
          `padding-left:${match[1]}${unit};padding-right:${match[1]}${unit};`,
      ],
      [
        /^p-(\d+)-(\d+)-(\d+)-(\d+)$/,
        (match, { unit }) => ({
      ...
      [
        /^(width|w)-(\d+)$/,
        (match, { unit }) => ({ width: `${match[2]}${unit}` }),
      ],
      [
        /^(width|w)-(\d+)(vh|vw|%)$/,
        (match, { unit }) => ({ width: `${match[2]}${match[3]};` }),
      ],
      [
        /^(height|h)-(\d+)(vh|vw|%)$/,
        (match, { unit }) => ({ height: `${match[2]}${match[3]};` }),
      ],
      [
        /^(height|h)-(\d+)$/,
        (match, { unit }) => ({ height: `${match[2]}${unit}` }),
      ],
      [
        /^br-(\d+)$/,
        (match, { unit }) => ({ "border-radius": `${match[1]}${unit}` }),
      ],
      [
        /^br-t(\d+)$/,
        (match, { unit }) =>
          `border-top-left-radius:${match[1]}${unit};border-top-right-radius:${match[1]}${unit};`,
      ],
      [
        /^br-b(\d+)$/,
        (match, { unit }) =>
          `border-bottom-left-radius:${match[1]}${unit};border-bottom-right-radius:${match[1]}${unit};`,
      ],
      {
        ...
        "line-dark":
          " border-bottom: 2px solid #797B89; width: 100%;height: 2px;",
